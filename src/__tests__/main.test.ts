  ...
  describe('configureOciCli', () => {
    ...
    const errorTestCases: [string, () => void, string][] = [
      ...
      [
        ...
        () => { (fs.mkdir as jest.MockedFunction<typeof fs.mkdir>).mockRejectedValueOnce(new Error('Permission denied')); },
    ...
    test.each(errorTestCases)('%s', async (description: string, setup: () => void, expectedError: string) => {
      setup();
      await expect(configureOciCli(mockPlatform, testConfig)).rejects.toThrow(expectedError);
    });
    ...
    const profileTestCases: [string, string | undefined, string][] = [
      ...
      ['should create DEFAULT profile when not specified', undefined, '[DEFAULT]']
    ];

    test.each(profileTestCases)('%s', async (description: string, profile: string | undefined, expectedHeader: string) => {
      if (profile) {
    ...
    it('should create profile-specific directories and files', async () => {
      ...
      expect(fs.mkdir).toHaveBeenCalledWith(expect.stringContaining('/TESTPROF'), { recursive: true });
      
      const writeCalls = (fs.writeFile as jest.MockedFunction<typeof fs.writeFile>).mock.calls;
      expect(writeCalls.some(call => String(call[0]).includes('TESTPROF/private_key.pem'))).toBe(true);
      expect(writeCalls.some(call => String(call[0]).includes('TESTPROF/session'))).toBe(true);
    });
